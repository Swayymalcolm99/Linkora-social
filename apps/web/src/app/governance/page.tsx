    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          {loading ? (
            <div className="py-12 text-center text-[var(--text-muted)] animate-pulse">Loading proposals...</div>
          ) : activeTab === "History" ? (
            <div className="space-y-4">
              {executedProposals.length === 0 ? (
                ...
                executedProposals.map((p) => (
                  <div key={p.id.toString()} className="flex justify-between items-center p-4 border border-[var(--border)] rounded-xl bg-[var(--muted)]/40">
                    <div>
                      <p className="font-semibold text-[var(--foreground)]">{p.parameter}</p>
                      <p className="text-sm text-[var(--text-muted)]">Changed to {p.new_value.toString()}</p>
                    </div>
            ...
            <div className="space-y-4">
              {displayedProposals.length === 0 ? (
                ...
                displayedProposals.map((p) => (
                  <div key={p.id.toString()} className="border border-[var(--border)] rounded-xl p-5 bg-[var(--background)] shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      <div className="p-3 rounded-lg bg-[var(--muted)]/40 border border-[var(--border)]">
                        <p className="text-xs text-[var(--text-muted)] uppercase tracking-wider mb-1">New Value</p>
                        <p className="text-lg font-semibold text-[var(--foreground)]">{p.new_value.toString()}</p>
                      </div>
                      <div className="p-3 rounded-lg bg-[var(--muted)]/40 border border-[var(--border)]">
                        <p className="text-xs text-[var(--text-muted)] uppercase tracking-wider mb-1">Effective Quorum</p>
                        <p className="text-lg font-semibold text-[var(--foreground)]">{p.effectiveQuorum}</p>
                      </div>
                    </div>
                      ...
                      {connected && p.status === GovStatus.Active && (
                        <div className="flex gap-2">
                          <button
                            ...
                            className="px-4 py-2 bg-green-600/20 text-green-500 hover:bg-green-600/30 border border-green-600/50 rounded-lg transition-colors text-sm font-medium"
                          >
                            Vote For
                          </button>
                          <button
                            onClick={() => handleVote(p.id, false)}
                            className="px-4 py-2 bg-red-600/20 text-red-500 hover:bg-red-600/30 border border-red-600/50 rounded-lg transition-colors text-sm font-medium"
                          >
                            Vote Against
                          </button>
                        </div>
        ...
        <div className="space-y-6">
          <div className="border border-[var(--border)] rounded-xl p-5 bg-[var(--background)] shadow-sm sticky top-24">
            ...
            {!connected ? (
              <p className="text-[var(--text-muted)] text-sm">Connect your wallet to create a proposal.</p>
            ) : (
